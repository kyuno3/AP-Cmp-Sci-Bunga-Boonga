...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("file")){
				...
				System.out.println("What word or phrase would you like to check the file for?");
				String check = sc.nextLine();
				System.out.println(check + " appears in "+ textFile +" "+checksArray(arr, check) + " times.");
	...
	*/
	public static void printsNumber(String[] arr, int num){
		if(num>arr.length){
			System.out.println("Too large of a number. Returning to main...\n");
		}
		else if (num<=0){
			System.out.println("Number of words printed can't be less than 0. Returning to main...\n");
		}
		else{
			for (int c = 0; c<num; c++){
				System.out.print(arr[c]+" ");
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		return checkCount;	// Dummy value, delete when needed.
	}
